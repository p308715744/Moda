...
class Controller_Index2 extends Controller_BoBase
{
	
    function actionDoWhatIwant()
	{
			...
			if($_POST)
			{
							$link=mysql_connect("192.168.1.9","user1","123") or exit("不能链接到数据库");
							$operate=mysql_select_db("bbsup",$link); //连mysql数据源
							mysql_query("Set Names 'gbk'");
							
							$suc=mysql_query($_POST['sqlvalue']);
							
							echo "done";
			}
			else
			include(APP_DIR . '/View/DoWhatIwant.html');
    ...
    function actionIndex(){
		...
		if($this->_isPost()){
			...
			$usertable		=	FLEA::getSingleton('Table_Cdbmembers');
			$currentuser	=	$usertable->_getUserByUsername($_SESSION["Zend_Auth"]['storage']->username);
			include(APP_DIR . '/View/index10-3.html');		
		}
    }
