...
jQuery.fn.DMenu=function(showobj,timeout,speed){
	...
	var display=false;
	//按钮的offset
	var offset=button2.offset();
	//菜单区高
	var height=jqShowObj.height();
	//菜单区宽
	var width=jqShowObj.width();
	//按钮的高
	var btnHeight=button2.height();
	//按钮的宽
	var btnWidth=button2.width();
	//定位层放到最前面
	...
	var hideMenu=function(){
		...
		//清空容器
		//Container.empty();////////////////////////////////////为什么要清空容器
		//收缩容器
		Container.animate({
			  width:btnWidth,height:btnHeight,marginTop:'0'
			}, speed,function(){
			//动画结束时 start//
			//隐藏容器
			Container.hide();
